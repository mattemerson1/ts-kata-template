export const booleanCalculator = (input:string):boolean | void => {
...
const formatInput = (input:string):string => {
    let formattedInput:string[] = Array.from(input);
...
const evaluator = (statementArray:string[]):string[] => {
    let parenthesesEvaluated = evaluateParentheses(statementArray)
    let notEvalulated = evaluateNot(parenthesesEvaluated);
    let andEvaluated = evaluateAnd(notEvalulated);
    let orEvaluated = evaluateOr(andEvaluated);
...
const evaluateParentheses = (statementArray:string[]):string[] => {
    let firstParentheses:number = 0;
    let secondParentheses:number = 0;
    for (let i=0;i<statementArray.length;i++) {
        if(statementArray[i] == "(") {
          firstParentheses = i;
        } else if (statementArray[i] == ')') {
          secondParentheses = i;
          let innerStatement = evaluator(statementArray.slice(firstParentheses + 1,secondParentheses));
          statementArray.splice(firstParentheses, (secondParentheses - firstParentheses) + 1, innerStatement[0])
        }
    }
...
const evaluateAnd = (statementArray:string[]):string[] => {
  for (let i=0;i<statementArray.length;i++) {
    if (statementArray[i] == "AND" && (statementArray[i - 1] == "TRUE" && statementArray[i + 1] == "TRUE")) {
      statementArray[i] = "TRUE";
      statementArray.splice(i - 1,1);
      statementArray.splice(i,1);
    } else if (statementArray[i] == "AND" && (statementArray[i - 1] == "FALSE" || statementArray[i + 1] == "FALSE")) {
      statementArray[i] = "FALSE";
      statementArray.splice(i - 1,1);
      statementArray.splice(i,1);
    };
...
const evaluateOr = (statementArray:string[]):string[] => {
  for (let i=0;i<statementArray.length;i++) {
    if (statementArray[i] == 'OR' && (statementArray[i - 1] == "TRUE" || statementArray[i + 1] == "TRUE")) {
      statementArray[i] = "TRUE";
      statementArray.splice(i - 1,1);
      statementArray.splice(i,1);
    } else if (statementArray[i] == 'OR' && (statementArray[i - 1] == "FALSE" && statementArray[i + 1] == "FALSE")) {
      statementArray[i] = "FALSE";
      statementArray.splice(i - 1,1);
      statementArray.splice(i,1);
    };
